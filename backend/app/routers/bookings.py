...
def create_booking(
    ...
    if not client:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Client not found. Add client first.")
    if client.status == ClientStatus.suspended:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Suspended clients cannot make new bookings.")
    if client.is_archived:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Archived clients cannot make new bookings.")
    ...
    if vehicle.is_archived:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Archived vehicles cannot be booked.")
...
def update_booking(
    ...
    if booking.is_archived:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Archived bookings cannot be edited")
    if booking.status in (BookingStatus.completed, BookingStatus.cancelled):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Completed or cancelled bookings cannot be edited")
...
def confirm_booking(
    ...
    if booking.status != BookingStatus.pending:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Only pending bookings can be confirmed. Current status: '{booking.status.value}'")
...
def activate_booking(
    ...
    if booking.status != BookingStatus.confirmed:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Only confirmed bookings can be activated. Current status: '{booking.status.value}'")
...
def complete_booking(
    ...
    if booking.status != BookingStatus.active:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Only active bookings can be completed. Current status: '{booking.status.value}'")
...
def no_show_booking(
    ...
    booking = _get_booking_or_404(booking_id, current_user.tenant_id, db)
    if booking.status != BookingStatus.confirmed:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Only confirmed bookings can be marked as no-show. Current status: '{booking.status.value}'")
...
def archive_booking(
    ...
    booking = _get_booking_or_404(booking_id, current_user.tenant_id, db)
    if booking.status == BookingStatus.active:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Active bookings cannot be archived")
    if booking.is_archived:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Booking is already archived")
    booking.is_archived = True
...
def restore_booking(
    ...
    if not booking.is_archived:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Booking is not archived")
...
def delete_booking(
    ...
    booking = _get_booking_or_404(booking_id, current_user.tenant_id, db)
    if booking.status == BookingStatus.active:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Active bookings cannot be deleted. Complete or cancel first.")
    db.delete(booking)
    db.commit()