...
def _require_super_admin(current_user: User) -> None:
    if current_user.role != UserRole.super_admin:
        raise HTTPException(
            ...
            detail="Only super admins can access this report",
        )
...
def _require_admin(current_user: User) -> None:
    if current_user.role not in (UserRole.super_admin, UserRole.tenant_admin):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only admins can access reports",
        )
...
def booking_summary_report(
    ...
    return {"detail": "PDF not available for booking summary — use excel or json"}
