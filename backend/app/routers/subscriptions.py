...
def _require_super_admin(current_user: User) -> None:
    if current_user.role != UserRole.super_admin:
        raise HTTPException(
            ...
            detail="Only super admins can manage subscriptions",
        )


def _get_subscription_or_404(subscription_id: int, db: Session) -> Subscription:
...
def get_subscription(
    ...
    if current_user.role != UserRole.super_admin:
        if sub.tenant_id != current_user.tenant_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You can only view your own tenant's subscriptions",
            )
    return sub
